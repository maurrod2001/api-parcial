...
app.post("/pacient", (req: { body: { no; name; lastname; gender; day_of_birth; email; phone; medicaments; alergy; }; }, res) => {
...
app.put("/pacient/:no", (req: { params: { no; }; body: { name; lastname; gender; day_of_birth; email; phone; medicaments; alergy; }; }, res) => {
  const { no } = req.params;
...
app.delete("/pacient/:no", (req: { params: { no; }; }, res) => {
  const { no } = req.params;
  const sql = `DELETE FROM pacient WHERE id = (SELECT id FROM pacient WHERE no =${no})`;
  connection.query(sql, (error) => {
    if (error) throw error;
...
app.get("/login/:username",(req,res)=>{
  ...
  const sql = `SELECT username , password FROM user WHERE id = (SELECT id FROM user WHERE username = '${username}')`
  connection.query(sql, (error, results) => {
    if (error) throw error;
    if (results.length > 0) {
      res.json(results);
